/**
 * StockRoom seed (TypeScript entrypoint, run via `npm run prisma:seed`).
 *
 * KEEP IN SYNC with prisma/seed/seed.js — that plain-JS twin is the production
 * path (`npx prisma db seed` / container start), this one is for local dev.
 *
 * Idempotent: every row is upserted under a deterministic id, so re-running
 * never duplicates. Guarded by SEED_ON_BOOT !== 'false'.
 *
 * Fixtures mirror the approved mockup exactly, so the seeded API reproduces the
 * designed screens 1:1 (per-location quantities sum to each item's totalOnHand).
 */
...
const USERS: Array<{ id: string; email: string; role: Role }> = [
  { id: 'usr-mgr', email: 'manager@demo', role: Role.manager },
  { id: 'usr-clk', email: 'clerk@demo', role: Role.clerk },
];
...
/**
 * Opening balances. Per-item sums are the mockup's totalOnHand:
